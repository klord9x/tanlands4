...
(function ($) {
    wpmfFoldersTreeModule = {
        ...
        importCategories: function () {
            ...
            if (parseInt(wpmf.vars.wpmf_active_media) === 1 && wpmf.vars.wpmf_role !== 'administrator' && (wpmf.vars.access_type === 'role' || wpmf.vars.access_type === 'user')) {
                folders_ordered_deep.push(wpmfFoldersModule.categories[wpmf.vars.term_root_id]);
            }
            let processed_ids = [];
        ...
        getRendering: function () {
            ...
            content = '<a class="wpmf-new-folder" onclick="wpmfFoldersModule.newFolder(wpmfFoldersModule.last_selected_folder)"><i class="material-icons">add</i>' + wpmf.l18n.create_folder + '</a>' + search_folder + content + loadmore;
